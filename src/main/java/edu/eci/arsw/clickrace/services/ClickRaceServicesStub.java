...
public class ClickRaceServicesStub implements ClickRaceServices {
    ...
    public int getWinner(int racenum) throws ServicesException {
        Set<RaceParticipant> carSet = racesData.get(racenum);
        for(RaceParticipant racer : carSet){
            if(racer.isWinner()) return racer.getNumber();
        }
        throw new ServicesException("No tenemos Ganador");
    }

    public void setWinner(int racenum, RaceParticipant rp){
        Set<RaceParticipant> carSet = racesData.get(racenum);
        for(RaceParticipant racer : carSet){
            if (racer.getNumber() == rp.getNumber()){
                racer.setWinner();
            }
        }
    }
